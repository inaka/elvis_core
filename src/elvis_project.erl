...
-type protocol_for_deps_erlang_mk_config() :: #{ignore => [module()], regex => string()}.
...
protocol_for_deps_erlang_mk(_Config, Target, RuleConfig) ->
    ...
    BadDeps = lists:filter(fun(Dep) -> is_erlang_mk_not_git_dep(Dep, Regex) end, Deps),
    lists:flatmap(fun(Line) -> erlang_mk_dep_to_result(Line, ?DEP_NO_GIT, {IgnoreDeps, Regex})
                  end,
                  BadDeps).

-type protocol_for_deps_rebar_config() :: #{ignore => [module()], regex => string()}.
...
protocol_for_deps_rebar(_Config, Target, RuleConfig) ->
    ...
    NoHexDeps = lists:filter(fun(Dep) -> not is_rebar_hex_dep(Dep) end, Deps),
    BadDeps = lists:filter(fun(Dep) -> is_rebar_not_git_dep(Dep, Regex) end, NoHexDeps),
    lists:flatmap(fun(Line) -> rebar_dep_to_result(Line, ?DEP_NO_GIT, {IgnoreDeps, Regex})
                  end,
                  BadDeps).
...
is_rebar_hex_dep({_AppName, _Vsn, {pkg, _PackageName}})
    ...
    true;
is_rebar_hex_dep({_AppName, _Vsn}) when is_atom(_AppName), is_list(_Vsn) ->
    true;
is_rebar_hex_dep(_) ->
    false.
...
is_erlang_mk_not_git_dep(Line, Regex) ->
    ...
    [_Protocol, Url | _] =
        [Part || Part <- binary:split(Dependency, <<" ">>, [global, trim]), Part /= <<>>],
    nomatch == re:run(Url, Regex, []).
...
erlang_mk_dep_to_result(Line, Message, {IgnoreDeps, Regex}) ->
    ...
    case lists:member(NameAtom, IgnoreDeps) of
        true ->
            [];
        false ->
            [elvis_result:new(item, Message, [Name, Regex])]
    end;
