...
check_no_macro_calls(Calls) ->
    TypeFun =
        fun(Call) ->
            FunctionSpec = elvis_ktn:function(Call),
            ModuleAttr = elvis_ktn:module(FunctionSpec),
            FuncAttr = elvis_ktn:function(FunctionSpec),
...
is_operator_node(Node) ->
    is_op(Node) andalso length(ktn_code:content(Node)) > 1.
...
invalid_dynamic_call(Config, Target, RuleConfig) ->
    ...
    case elvis_code:find(fun is_callback_attr/1, Root) of
        [] ->
...
max_anonymous_function_arity(Config, Target, RuleConfig) ->
    ...
    IsFun =
        fun(Node) ->
            ...
            is_fun(Node) andalso [] /= elvis_code:find(fun is_clause_node/1, Node)
        end,
...
node_line_limits(FunctionNode) ->
    ...
    LineNums = zipper:map(fun elvis_ktn:line/1, Zipper),
...
is_relevant_behaviour(Root, RuleConfig) ->
    ...
    Behaviours = elvis_code:find(fun is_behaviour_attr/1, Root),
    lists:any(
        fun(Elem) -> Elem end,
        lists:map(
            fun(BehaviourNode) ->
...
is_ets_fun2ms(Node) ->
    Fun = elvis_ktn:function(Node),
    Fun2 = elvis_ktn:function(Fun),
...
behaviour_spelling(Config, Target, RuleConfig) ->
    ...
    Predicate =
        fun(Node) ->
            is_behaviour_attr(Node) andalso ktn_code:type(Node) /= Spelling
        end,
    case elvis_code:find(Predicate, Root) of
        [] ->
            [];
...
is_otp_module(Root) ->
    ...
    case elvis_code:find(fun is_behaviour_attr/1, Root) of
        [] ->
            false;
        Behaviors ->
...
call_mfa(Call) ->
    ...
    F = elvis_ktn:value(elvis_ktn:function(FunctionSpec)),
