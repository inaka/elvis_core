...
verify_required_patterns(_Config) ->
    ...
    [] = elvis_gitignore:required_patterns(
        {elvis_gitignore, GitIgnoreConfig, FilePass, NoRuleConfig}
    ),

    PathFail = ".gitignore",
    {ok, FileFail} = elvis_test_utils:find_file([SrcDirFail], PathFail),
    [Res] = elvis_gitignore:required_patterns(
        {elvis_gitignore, GitIgnoreConfig, FileFail, NoRuleConfig}
    ),
    #{info := ["^doc/$"]} = Res.
...
verify_forbidden_patterns(_Config) ->
    ...
    PathFail = ".gitignore",
    {ok, FileFail} = elvis_test_utils:find_file([SrcDirFail], PathFail),
    [Res] = elvis_gitignore:forbidden_patterns(
        {elvis_gitignore, GitIgnoreConfig, FileFail, NoRuleConfig}
    ),
    #{info := ["^rebar.lock$"]} = Res.
